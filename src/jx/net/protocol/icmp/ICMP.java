...
import jx.net.protocol.arp.ARP;
...
public class ICMP implements MemoryConsumer, IPConsumer {
	...
	Debug.out.println("ICMP packet received: offset=");
        ICMPFormat icmp = new ICMPFormat(buf, data.getOffset());
	...
	if (type == TYPE_ECHO) {
	    Debug.out.println("  ICMP ECHO RECEIVED. (ping)");
            ...
            int seq = icmp.getSequenceNumber();
            icmp.insertType((byte)0);
            icmp.setIdentifier(id);
            icmp.setSequenceNumber(seq);
            icmp.CalculateChecksum();
            // Use original buffer which has 34 bytes leading space (14 Ether + 20 IP)
            try {
                sender = net.getIPSender(data.getSourceAddress(), 1);
                sender.send(buf);
            } catch (Exception ex) {
                //Debug.out.println("  ARP: queuing ICMP reply for later delivery: " + ex);
                ARP arp = (ARP)ipLayer.getAddressResolution();
                if (arp != null) {
                    arp.queuePacket(data.getSourceAddress().getBytes(), buf);
                }
            }
	}
	return buf;
