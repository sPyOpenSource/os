...
public class IPv4Utils {
     ...
     */
    public static short calcChecksum(Memory skbuf, int start, int length) {
     ...
     * @return The calculated checksum
     */
    public static short calcChecksum(Memory skbuf, int start, int length, int initialValue) {
        final int size = skbuf.size();
        int chsum = ~initialValue;
        for (int i = 0; i < length; i += 2) {
            final int v;
            if (i + 1 >= size) {
                v = (skbuf.get8(start + i) << 8);
            } else {
                v = skbuf.get16((start + i) >> 1);
            }
            chsum += v;
